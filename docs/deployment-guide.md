...
# API=$BASE/api/v1 — /health만 예외적으로 $BASE를 그대로 쓴다(버전과 무관한 고정 경로)
BASE=http://127.0.0.1:7000  API=$BASE/api/v1  ADMIN=paas_...
...
- Payload URL: `https://paas.example.com/api/v1/webhooks/git`
...
프록시는 아예 건드리지 않는다. `GET /api/v1/server-config`·콘솔 "서버구성" 토폴로지
다이어그램에서 컴포넌트별 상태를 개별 확인할 수 있다.

`POST /api/v1/projects/1/rollback`도 컴포넌트 단위가 아니라 backend/frontend가 **한 쌍으로
