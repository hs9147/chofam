...
모든 엔드포인트는 `/api/v1` 아래 마운트된다 — `/health`, `/status`만 예외
(로드밸런서/k8s liveness probe, 콘솔 로그인 프로브가 버전과 무관한 고정 경로를 기대함,
`app/main.py`의 `API_PREFIX` 참고).

```
GET  /health                          # 인증 불필요, prefix 없음
GET  /status                          # CPU/메모리/디스크/GPU (admin), prefix 없음
POST /api/v1/keys                     # API 키 발급 (admin)
GET  /api/v1/audit                    # 감사 로그 (admin)

POST /api/v1/orgs                     # {name} → 사내 Gitea에 동명 Organization 생성 (admin)
GET  /api/v1/orgs                     # 조직 목록 + 프로젝트 수

GET  /api/v1/projects                 # git_url은 organization_id 소속이면 비관리자에게 마스킹
POST /api/v1/projects                 # {name, type, branch, domain?, ...}
                                      #   + organization_id(내부 리포 자동 생성) 또는 git_url(직접 지정) 중 하나
POST /api/v1/projects/upload          # multipart: zip_file 또는 files[](폴더) 중 하나 + organization_id 필수
                                      #   업로드 내용을 사내 Gitea 신규 리포에 최초 push (대용량/zip bomb/zip slip 방어)
                                      #   deploy_after_upload=true면 push 직후 배포 큐에 등록(원클릭)
GET  /api/v1/projects/{id}/files      # 읽기 전용 파일 트리 (workspace 기능)
GET  /api/v1/projects/{id}/files/content?path=  # 읽기 전용 파일 내용 — 수정 엔드포인트는 없음(채팅/diff로만)
POST /api/v1/projects/{id}/deploy     # {profile?: development|release, git_sha?}
POST /api/v1/projects/{id}/rollback?profile=release
POST /api/v1/projects/{id}/stop?profile=development
GET  /api/v1/projects/{id}/deployments
GET  /api/v1/projects/{id}/logs?profile=release&tail=200
GET  /api/v1/projects/{id}/status
PUT  /api/v1/projects/{id}/env        # {key, value, is_secret} — Fernet 암호화 저장
GET  /api/v1/projects/{id}/env        # 시크릿 값은 마스킹

GET  /api/v1/server-config            # 서버구성 시각화 — 런타임/프록시 백엔드 + 프로젝트별
                                      #   (프로필별) 도메인·상태·리다이렉트 규칙 수
POST /api/v1/projects/{id}/redirects  # {from_path, to_path, kind: redirect|rewrite, status_code?}
                                      #   다음 배포·롤백부터 리버스프록시 설정에 반영
GET  /api/v1/projects/{id}/redirects
DELETE /api/v1/redirects/{id}

POST /api/v1/webhooks/git             # GitHub/Gitea push → default_profile로 자동 배포
...
```bash
ADMIN=... BASE=http://localhost:7000/api/v1
...
```
POST /api/v1/llm/providers                     # 외부(Claude/OpenAI) 또는 내부(project://<llm 프로젝트>) 등록 (admin)
GET  /api/v1/llm/providers                     # api_key는 has_api_key로만 노출
POST /api/v1/chat/sessions                     # {project_id, provider_id, branch?} — 기본 브랜치 paas/chat-{id}
POST /api/v1/chat/sessions/{id}/messages       # 파일 트리·모듈 규약·요청 파일을 컨텍스트로 LLM 호출
                                                # 응답에 diff가 있으면 ProposedChange 자동 생성
POST /api/v1/changes/{id}/apply                # 승인 → 작업 브랜치에 git apply + commit (LLM 직접 쓰기 없음)
POST /api/v1/changes/{id}/reject
POST /api/v1/projects/{id}/review              # {provider_id, diff? , base_ref?} → 심각도 분류 findings

POST /api/v1/modules                           # external_api | internal_api | database | file_storage
                                                #   + category?(예: news, llm — API 카테고리별 그룹핑)
                                                #   + organization_id?(지정 시 해당 조직 프로젝트에만 노출)
                                                # config의 api_key/dsn/secret 등은 Fernet 암호화 저장
POST /api/v1/projects/{id}/modules/{mid}/bind  # {env_prefix: "PAY"} → 배포 시 PAY_URL 등 자동 주입
GET  /api/v1/projects/{id}/modules             # LLM 컨텍스트용 요약 (비밀값 제외, 바인딩된 모듈만)
GET  /api/v1/projects/{id}/resources           # 대화식 편집 화면 자원 리스팅 — 바인딩 여부와 무관하게
                                                #   이 프로젝트에서 쓸 수 있는 모든 모듈을 카테고리별로 아이템화

POST /api/v1/projects/{id}/preview             # {branch?, ttl_minutes=60} → {name}-pv{n}.{base_domain}
GET  /api/v1/projects/{id}/previews            # 조회 시 만료 프리뷰 자동 회수
...
- **조직별 작업공간**: 콘솔의 조직 페이지(admin)에서 조직을 만들면 사내 Gitea에 동일한
  ...
  응답에서 마스킹**된다(admin만 실제 값 조회 가능) — `POST /api/v1/orgs`, `GET /api/v1/orgs`,
  `POST /api/v1/projects`의 `organization_id` 참고.
- **zip/폴더 업로드 등록**: `POST /api/v1/projects/upload`(조직 필수) — git 저장소가 아직 없는
...
- **코드 확인 화면**: `GET /api/v1/projects/{id}/files`, `/files/content`로 리포를 읽기 전용
  브라우징. 저장/수정 엔드포인트는 존재하지 않으며, 실제 코드 변경은 항상 LLM 채팅 →
  diff 제안 → `POST /api/v1/changes/{id}/apply` 승인 경로로만 이뤄진다(12절 원칙 유지).
- **웹훅 자동 등록**: `PAAS_PLATFORM_PUBLIC_URL` 설정 시 조직 소속/업로드로 리포를 만들
  때마다 플랫폼이 자신의 `/api/v1/webhooks/git`을 Gitea 웹훅으로 자동 등록한다(베스트 에포트 —
...
- **운영환경별 런타임/리버스프록시 선택 + 서버구성 시각화**: 1차(small)는 실행 런타임을
  ...
  등록·반영한다. `GET /api/v1/server-config`가 현재 선택된 백엔드와 프로젝트별(프로필별)
  도메인·실행 상태·리다이렉트 규칙 수를 한 화면에서 보여주고, 콘솔 "서버구성" 메뉴는
  이를 표와 함께 proxy → 사이트 → runtime 관계를 그리는 토폴로지 다이어그램(순수 SVG,
  신규 의존성 없음)으로도 시각화한다.
- **프로젝트별 URL redirect/rewrite 규칙**: `POST/GET /api/v1/projects/{id}/redirects`,
  `DELETE /api/v1/redirects/{id}`로 등록하면 다음 배포·롤백 때 선택된 프록시 백엔드 설정에
...
- **비동기 배포**: `POST /api/v1/projects/{id}/deploy`에 `"wait": false` → 202 즉시 반환,
  `GET /api/v1/projects/{id}/deployments`로 진행 폴링. 워커 수는 `PAAS_DEPLOY_WORKERS`(기본 2).
...
- **키 회전**: 새 키를 `PAAS_FERNET_KEY`로, 기존 키를 `PAAS_FERNET_KEYS_OLD`로 옮겨
  재기동 → `POST /api/v1/admin/rotate-secrets`(admin) → 완료 후 구 키 제거.
