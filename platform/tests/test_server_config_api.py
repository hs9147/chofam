...
def test_server_config_defaults(monkeypatch, fresh_settings):
    ...
    r = c.get("/api/v1/server-config", headers=ADMIN)
...
def test_server_config_reflects_backend_settings(monkeypatch, fresh_settings):
    ...
    c = _client()
    body = c.get("/api/v1/server-config", headers=ADMIN).json()
...
def test_redirect_crud_flow(fresh_settings):
    ...
    r = c.post(f"/api/v1/projects/{pid}/redirects", json={
    ...
    listing = c.get(f"/api/v1/projects/{pid}/redirects", headers=ADMIN).json()
    assert len(listing) == 1

    server_cfg = c.get("/api/v1/server-config", headers=ADMIN).json()
    release = next(s for s in server_cfg["sites"] if s["project_id"] == pid and s["profile"] == "release")
    assert release["redirect_count"] == 1

    assert c.delete(f"/api/v1/redirects/{rule['id']}", headers=ADMIN).status_code == 204
    assert c.get(f"/api/v1/projects/{pid}/redirects", headers=ADMIN).json() == []
...
def test_redirect_unknown_project_404(fresh_settings):
    c = _client()
    r = c.post("/api/v1/projects/999999/redirects", json={"from_path": "/a", "to_path": "/b"}, headers=ADMIN)
    assert r.status_code == 404
    assert c.get("/api/v1/projects/999999/redirects", headers=ADMIN).status_code == 404


def test_delete_unknown_redirect_404(fresh_settings):
    c = _client()
    assert c.delete("/api/v1/redirects/999999", headers=ADMIN).status_code == 404
