...
def test_deploy_only_hides_other_modules(monkeypatch, fresh_settings):
    ...
    # core는 살아있음
    assert c.get("/api/v1/projects", headers=ADMIN).status_code == 200
    assert c.get("/api/v1/modules", headers=ADMIN).status_code == 200
    # 비활성 모듈 라우터는 미마운트 → 404
    assert c.get("/api/v1/llm/providers", headers=ADMIN).status_code == 404
    assert c.get("/api/v1/payments", headers=ADMIN).status_code == 404
    assert c.post("/api/v1/chat/sessions", json={"project_id": 1, "provider_id": 1},
...
def test_deploy_endpoints_gated_when_disabled(monkeypatch, fresh_settings):
    ...
    pid = r.json()["id"]
    assert c.post(f"/api/v1/projects/{pid}/deploy", json={}, headers=ADMIN).status_code == 404
    assert c.get(f"/api/v1/projects/{pid}/deployments", headers=ADMIN).status_code == 404
    # 웹훅·프리뷰·서버구성 라우터도 미마운트
    assert c.post("/api/v1/webhooks/git", content=b"{}").status_code == 404
    assert c.get(f"/api/v1/projects/{pid}/previews", headers=ADMIN).status_code == 404
    assert c.get("/api/v1/server-config", headers=ADMIN).status_code == 404
