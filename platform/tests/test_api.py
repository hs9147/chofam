...
def test_requires_api_key():
    ...
    assert c.get("/api/v1/projects", headers={"x-api-key": "wrong"}).status_code == 401
...
def test_project_crud_and_env_masking():
    ...
    r = c.post("/api/v1/projects", json=body, headers=ADMIN)
    assert r.status_code == 201, r.text
    pid = r.json()["id"]
    assert r.json()["default_profile"] == "release"

    assert c.post("/api/v1/projects", json=body, headers=ADMIN).status_code == 409

    r = c.put(f"/api/v1/projects/{pid}/env", json={"key": "API_TOKEN", "value": "s3cret"}, headers=ADMIN)
    assert r.status_code == 204
    r = c.get(f"/api/v1/projects/{pid}/env", headers=ADMIN)
    assert r.json() == [{"key": "API_TOKEN", "is_secret": True, "value": "•••"}]


def test_issue_key_and_use_it():
    c = _client()
    r = c.post("/api/v1/keys", json={"name": "ci-bot"}, headers=ADMIN)
    assert r.status_code == 201
    issued = r.json()["key"]
    assert issued.startswith("paas_")
    assert c.get("/api/v1/projects", headers={"x-api-key": issued}).status_code == 200
    # 일반 키로는 관리자 엔드포인트 접근 불가
    assert c.get("/api/v1/audit", headers={"x-api-key": issued}).status_code == 403
...
def test_webhook_signature_required():
    ...
    r = c.post("/api/v1/webhooks/git", content=raw, headers={"x-hub-signature-256": "sha256=bad"})
    assert r.status_code == 401

    sig = hmac.new(b"test-webhook-secret", raw, hashlib.sha256).hexdigest()
    r = c.post(
        "/api/v1/webhooks/git", content=raw,
...
def test_audit_trail_recorded():
    ...
    rows = c.get("/api/v1/audit", headers=ADMIN).json()
