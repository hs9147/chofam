...
def _create_provider(c: TestClient, name="claude") -> int:
    r = c.post("/api/v1/llm/providers", json={
...
def test_provider_key_never_exposed():
    c = _client()
    _create_provider(c)
    listing = c.get("/api/v1/llm/providers", headers=ADMIN).json()
...
def _member_key(c: TestClient, name="dev1") -> dict:
    key = c.post("/api/v1/keys", json={"name": name}, headers=ADMIN).json()["key"]
...
def test_non_admin_key_blocked_from_external_provider_session():
    ...
    r = c.post("/api/v1/chat/sessions", json={"project_id": pid, "provider_id": prov},
...
def test_non_admin_key_allowed_for_internal_provider_session():
    c = _client()
    pid = _create_project(c)
    prov_id = c.post("/api/v1/llm/providers", json={
        "name": "llm-internal", "kind": "internal", "base_url": "project://llm-internal",
        "model": "m",
    }, headers=ADMIN).json()["id"]
    r = c.post("/api/v1/chat/sessions", json={"project_id": pid, "provider_id": prov_id},
               headers=_member_key(c))
...
def test_non_admin_key_blocked_from_external_review():
    ...
    pid = _create_project(c)
    prov = _create_provider(c)  # external
    r = c.post(f"/api/v1/projects/{pid}/review",
...
def test_admin_key_still_allowed_for_external_session():
    ...
    r = c.post("/api/v1/chat/sessions", json={"project_id": pid, "provider_id": prov}, headers=ADMIN)
    assert r.status_code == 200
...
def test_chat_message_creates_proposed_change(monkeypatch):
    ...
    r = c.post(f"/api/v1/chat/sessions/{sid}/messages",
    ...
    assert c.post(f"/api/v1/changes/{cid}/reject", headers=ADMIN).status_code == 204
    assert c.post(f"/api/v1/changes/{cid}/apply", headers=ADMIN).status_code == 409
...
def test_chat_without_diff_makes_no_change(monkeypatch):
    ...
    pid = _create_project(c)
    prov = _create_provider(c)
    sid = c.post("/api/v1/chat/sessions", json={"project_id": pid, "provider_id": prov},
                 headers=ADMIN).json()["id"]
    body = c.post(f"/api/v1/chat/sessions/{sid}/messages",
...
def test_module_bind_and_llm_context():
    ...
    r = c.post(f"/api/v1/projects/{pid}/modules/{mid}/bind", json={"env_prefix": "MAIL"}, headers=ADMIN)
    assert r.status_code == 201
    assert r.json()["injected_env"] == ["MAIL_API_KEY", "MAIL_URL"]

    # 같은 prefix 재사용 금지
    assert c.post(f"/api/v1/projects/{pid}/modules/{mid}/bind",
                  json={"env_prefix": "MAIL"}, headers=ADMIN).status_code == 409

    ctx = c.get(f"/api/v1/projects/{pid}/modules", headers=ADMIN).json()
