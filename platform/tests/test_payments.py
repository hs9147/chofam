...
def test_confirm_records_payment(monkeypatch):
    ...
    c = _client()
    r = c.post("/api/v1/payments/confirm", json=CONFIRM, headers=ADMIN)
    ...
    listing = c.get("/api/v1/payments", headers=ADMIN).json()
...
def test_confirm_is_idempotent_but_conflicts_on_mismatch(monkeypatch):
    _mock_ok(monkeypatch)
    c = _client()
    assert c.post("/api/v1/payments/confirm", json=CONFIRM, headers=ADMIN).status_code == 200
    # 동일 내용 재시도 → 멱등 200
    assert c.post("/api/v1/payments/confirm", json=CONFIRM, headers=ADMIN).status_code == 200
    # 같은 orderId, 다른 금액 → 409
    bad = dict(CONFIRM, amount=99999)
    assert c.post("/api/v1/payments/confirm", json=bad, headers=ADMIN).status_code == 409
...
def test_cancel_flow_and_permissions(monkeypatch):
    _mock_ok(monkeypatch)
    c = _client()
    c.post("/api/v1/payments/confirm", json=CONFIRM, headers=ADMIN)

    member = c.post("/api/v1/keys", json={"name": "svc"}, headers=ADMIN).json()["key"]
    # 일반 키는 취소 불가
    r = c.post("/api/v1/payments/pk_test_1/cancel", json={"reason": "테스트"},
               headers={"x-api-key": member})
    assert r.status_code == 403

    r = c.post("/api/v1/payments/pk_test_1/cancel", json={"reason": "고객 변심"}, headers=ADMIN)
    assert r.status_code == 200
    assert r.json()["status"] == "canceled"
    # 이미 취소된 건 재취소 불가
    assert c.post("/api/v1/payments/pk_test_1/cancel", json={}, headers=ADMIN).status_code == 409
